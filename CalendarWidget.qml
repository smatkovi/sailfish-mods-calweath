...
Column {
    ...
    function refreshLocations() {
        locationHelper.typedCall("getUpcoming",
            [{"type": "i", "value": 14}, {"type": "i", "value": 50}],
            function(result) {
                var data = JSON.parse(result)
                var cache = {}
                for (var i = 0; i < data.length; i++) {
                    cache[data[i].key] = data[i].location
                }
                locationCache = cache
                console.log("Location cache loaded: " + Object.keys(cache).length + " entries")
            },
            function(error) { console.log("Location helper error: " + error) }
        )
    }

    CalendarEventsModel {
        id: eventsModel
        onCountChanged: calendarWidget.refreshLocations()
    ...
    Repeater {
        ...
        delegate: Column {
            ...
            CalendarWidgetDelegate {
                ...
                eventLocation: calendarWidget.locationCache[displayLabel.indexOf('HTM') >= 0 ? displayLabel.substring(0,30) : displayLabel.substring(0,20) + '_' + startTime.getMonth() + '_' + startTime.getDate() + '_' + startTime.getHours()] || ""
